(function (factory) {
  ...
  var Drawer = /*#__PURE__*/function () {
    ...
    _createClass(Drawer, [{
      ...
      value: function open(event) {
        // Check to see if we are trying to open the cart drawer... We don't want this
        // because we always want the full cart page
        if (event && event.type === 'click' && this.element.id === 'sidebar-cart') {
          return;
        }

        // Then of course we don't want to open the cart if it is already open
        if (this.isOpen) {
          return;
  ...
  var ProductVariants = /*#__PURE__*/function () {
    function ProductVariants(container, options) {
      ...
      this._updateOutOfStockVariants(null, null);
      this._updateVariantIdentifiers(null);
    }
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _onVariantChanged(previousVariant, newVariant) {
        ...
        this._updateOutOfStockVariants(newVariant, previousVariant);
        this._updateVariantIdentifiers(newVariant);
      ...
      key: "_updateVariantIdentifiers",
      value: function _updateVariantIdentifiers(newVariant) {
        if (!newVariant) {
          newVariant = this.currentVariant;
        }

        var jsonData = JSON.parse(this.element.querySelector('[data-product-json]').innerHTML);
        var productIdentifiers = jsonData['product_identifiers'];
        var variantIdentifiers = jsonData['variant_identifiers'];
        var newVariantIdentifiers = variantIdentifiers[newVariant.id];

        var identifierElements = [
          document.getElementById('product-vsn'),
          document.getElementById("product-variant-sku"),
          document.getElementById("product-variant-upc")
        ];

        var identifiers = [
          productIdentifiers.vsn,
          newVariantIdentifiers.sku,
          newVariantIdentifiers.upc
        ];

        identifierElements.forEach(function (element, index) {
          if (element) {
            element.innerHTML = identifiers[index];
          }
        });
      }
    }, {
      key: "_updateOutOfStockVariants",
      value: function _updateOutOfStockVariants(newVariant, previousVariant) {
        var _productData = this.productData;
        var _variantsInventories = this.variantsInventories;
        ...
        try {
          ...
          if (newVariant.options) {
            optionCount = newVariant.options.length || 0;
          }
  
          // Undo all the inventory cross-outage we did in the previous run
          // so we can start out fresh
          for (var i = 0; i < optionCount; ++i) {
            var optionSelectorQuery = 'button[data-option-position="' + (i + 1) + '"]';
            var optionButtons = document.querySelectorAll(optionSelectorQuery);
            optionButtons.forEach(function (button) {
              button.classList.remove("Popover__Value--unavailable");
            });
          }
  
          // Any product with more than 2 variants would get too complicated...
          if (optionCount == 1) {
            var optionIndex = 0;
            var optionSelectorQuery = 'button[data-option-position="' + (optionIndex + 1) + '"]';
            var optionButtons = document.querySelectorAll(optionSelectorQuery);
            optionButtons.forEach(function (button) {
              var optionValue = button.getAttribute("data-value");
              _productData.variants.forEach(function (variant) {
                if (optionValue === variant.options[optionIndex]) {
                  if (!variant.available) {
                    button.classList.add("Popover__Value--unavailable");
                  }
                }
              });
            });
          } else if (optionCount == 2) {
            // Products with 2 options
            var changedOptionIndex = 0;
            if (previousVariant) {
              for (var i = 0; i < optionCount; ++i) {
                if (newVariant.options[i] !== previousVariant.options[i]) {
            ...
            var otherOptionIndex = changedOptionIndex === 0 ? 1 : 0;
            var otherOptionSelectorQuery = 'button[data-option-position="' + (otherOptionIndex + 1) + '"]';
            var optionButtons = document.querySelectorAll(otherOptionSelectorQuery);
  
            // If there is only one choice for the other option, use the other
            // option for finding available products
            if (optionButtons.length == 1) {
              changedOptionIndex = changedOptionIndex === 0 ? 1 : 0;
              otherOptionIndex = otherOptionIndex === 0 ? 1 : 0;
  
              otherOptionSelectorQuery = 'button[data-option-position="' + (otherOptionIndex + 1) + '"]';
              optionButtons = document.querySelectorAll(otherOptionSelectorQuery);
            }
  
            var changedOptionValue = newVariant.options[changedOptionIndex];
            optionButtons.forEach(function (button) {
              var otherOptionValue = button.getAttribute("data-value");
              _productData.variants.forEach(function (variant, variantIndex) {
                if (changedOptionValue === variant.options[changedOptionIndex]
                  && otherOptionValue === variant.options[otherOptionIndex]) {
                  if (!variant.available) {
                    button.classList.add("Popover__Value--unavailable");
                  }
                }
      ...
      value: function _addToCart(event) {
        ...
        var formData = Form.serialize(formElement);
        fetch("".concat(window.routes.cartAddUrl, ".js"), {
          ...
          var errorMessageId = "add-to-cart-error-message";
          ...
          if (response.ok) {
            ...
            response.json().then(function (content) {
              console.log(_this4.variantsInventories);
              ...
              try {
                var requestedQuantity = parseInt(formData.quantity);
                var onHandQuantity = parseInt(_this4.variantsInventories[formData.id].inventory_quantity);
                if (requestedQuantity > onHandQuantity) {
                  quantityElement.value = "".concat(onHandQuantity);
                  errorMessageElement.innerHTML = "Sorry! There's only " + onHandQuantity + " of these items in stock. "
                    + "Try ordering that amount of product or less.";  
                } else {
  ...
  var SearchBar = /*#__PURE__*/function () {
    ...
    _createClass(SearchBar, [{
      ...
      value: function _onInput(event) {
        ...
        // CUSTOM: grab the permissions that should be a part of our query
        var augmentedInputValue = this.lastInputValue;
        var permissionSearchTerms = window.CgxPatronValidation.PermissionSearchTerms;
        if (permissionSearchTerms) {
          augmentedInputValue = "title:" + augmentedInputValue + "* AND (" + permissionSearchTerms + ")";
        } else {
          augmentedInputValue += "*";
        }
        
        var queries = [fetch("".concat(window.routes.searchUrl, "?section_id=predictive-search&q=").concat(encodeURIComponent(augmentedInputValue), "&type=product"), queryOptions)];
        if (window.theme.searchMode !== 'product') {
          queries.push(fetch("".concat(window.routes.searchUrl, "?section_id=predictive-search&q=").concat(encodeURIComponent(augmentedInputValue), "&type=").concat(window.theme.searchMode.replace('product,', '')), queryOptions));
        }

        this.queryMap[augmentedInputValue] = true;
        document.dispatchEvent(new CustomEvent('theme:loading:start'));
        Promise.all(queries).then(function (responses) {
          // If we receive the result for a query that is not the last one, we simply do not process the result
          if (augmentedInputValue !== event.target.value) {
            // return;
          }
  ...
  var CartSection = /*#__PURE__*/function () {
    ...
    _createClass(CartSection, [{
      ...
      value: function _attachListeners() {
        ...
        // We always want to show the cart drawer when adding to the cart
  ...
  var FeaturedProductSection = /*#__PURE__*/function () {
    ...
    _createClass(FeaturedProductSection, [{
      ...
      value: function _updateMainImage(event) {
        ...
        console.log("New image info: ", newImage);
  ...
  var ProductSection = /*#__PURE__*/function () {
    ...
    _createClass(ProductSection, [{
      ...
      value: function _updateSlideshowImage(event) {
        ...
        // CUSTOM: In Liquid, we only render photos that aren't repeats, so we'll have to
        // look up the media ID we'll need by ourselves.
        var disregardIndices = window.CgxPatronValidation.ProductVariant.DisregardProdctOptionIndices;
        var mediaRenderCache = window.CgxPatronValidation.ProductVariant.MediaRenderCache;
        var renderedOptionCombos = window.CgxPatronValidation.ProductVariant.RenderedOptionCombos;
        
        var variantOptionString = "";
        for (var i = 0; i < disregardIndices.length; i++) {
          var disregardVariant = disregardIndices[i] == "false";
          if (!disregardVariant) {
            variantOptionString += variant.options[i];
          }
        }

        var variantComboIndex = 0;
        var mediaId = "";

        try {
          variantComboIndex = renderedOptionCombos.indexOf(variantOptionString);
          mediaId = mediaRenderCache[variantComboIndex];
        } catch (error) {
          return;
        }
        ...
        if (Responsive.matchesBreakpoint('pocket') || !this.options['stackProductImages']) {
          ...
          var mediaElement = document.querySelector("[href=\"#Media".concat(mediaId, "\"]"));
          if (mediaElement) {
            mediaElement.click(); //document.getElementById(`Media${variant['featured_media']['id']}`).scrollIntoView({behavior: 'smooth'}); 
          }
        }
      }
      /**
