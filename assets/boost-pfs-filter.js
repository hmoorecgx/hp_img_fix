...
function userLoggedIn() {
  return !!window.CgxPatronValidation.PermissionTags 
    && !!window.CgxPatronValidation.PermissionTags.length;
}
...
function buildPrice(data) {
  ...
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') 
      ...
      if (userLoggedIn()) {
        ...
        html += '<span class="boost-pfs-filter-product-item-regular-price">' 
          + '(Log in to see price)' + '</span>';
      }
      html += '</p>';
  }
  return html;
...
jQ(document).ready(function() {
  // Don't use too much vertical space, jesus
  var searchWrapper = jQ('.boost-pfs-search-form-wrapper')[0];
  searchWrapper.style.paddingTop = '25px';
  searchWrapper.style.paddingBottom = '0px';
  
  // Fix issue header fix on the Parallax theme
  if(jQ('.mm-fixed-top').length > 0){
    var headerFixedHeight = jQ('.mm-fixed-top').height();
    var filterWrapperSelector = jQ('.boost-pfs-filter-collection-header-wrapper');
    filterWrapperSelector.css('margin-top', headerFixedHeight);
  }

  //
  var filterHeaderSelector = jQ('.boost-pfs-filter-option-title-text');
  filterHeaderSelector.addClass('Heading');
});
})();